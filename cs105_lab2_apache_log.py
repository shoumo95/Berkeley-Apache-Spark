# Databricks notebook source exported at Sat, 22 Oct 2016 22:54:41 UTC
# MAGIC %md
...
days_with_hosts = []
hosts = []
for row in daily_hosts_df.select('day').collect():
  days_with_hosts.append(row[0])
for row in daily_hosts_df.select('count').collect():
  hosts.append(row[0])
...
total_req_per_day_df = logs_df.select(dayofmonth('time').alias('day'),'host').groupBy('day').count()
...
days_with_avg = (avg_daily_req_per_host_df.map(lambda (x,y):x).collect())
avgs = (avg_daily_req_per_host_df.map(lambda (x,y):y).collect())
#for <FILL IN>:
#  <FILL IN>
...
top_20_not_found_df = not_found_paths_df.groupBy('path').count().sort(desc('count'))
...
days_with_errors_404 = errors_by_date_sorted_df.map(lambda(x,y):x).collect()
errors_404_by_day = errors_by_date_sorted_df.map(lambda(x,y):y).collect()
#for <FILL IN>:
#  <FILL IN>
...
hour_records_sorted_df = (not_found_df.select('path',hour('time').alias('hour')).groupBy('hour').count().cache())
...
hours_with_not_found = hour_records_sorted_df.map(lambda(x,y):x).collect()
not_found_counts_per_hour = hour_records_sorted_df.map(lambda(x,y):y).collect()
