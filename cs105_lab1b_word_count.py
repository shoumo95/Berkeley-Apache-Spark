# Databricks notebook source exported at Sat, 22 Oct 2016 22:54:21 UTC
# MAGIC %md
...
pluralLengthsDF = pluralDF.select(length('word')).alias('len')
...
def wordCount(wordListDF):
    ...
    """
    return wordListDF.groupBy('word').count().alias('count')
...
def removePunctuation(column):
    ...
    """
    punctuation_free = regexp_replace(column,'[^a-zA-Z0-9\s]+','')
    trimmed = trim(punctuation_free)
...
shakeWordsDF = (shakespeareDF
                .select(explode(split(shakespeareDF[0],' ')).alias('word'))).where(length('word') > 0)
